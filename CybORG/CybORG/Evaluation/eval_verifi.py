        ...
        for red_agent in [B_lineAgent, RedMeanderAgent, SleepAgent]:
            ...
            for i in range(MAX_EPS):
                ...
                for j in range(num_steps):
                    ...
                    base_threshold = 1e-4 
                    # Use dynamic threshold and enforce a minimum value
                    threshold = max(base_threshold, np.std(shap_values[0]) / 10)
                    print(f"Dynamic threshold for SHAP filtering: {threshold}")

                    # Filter significant SHAP values
                    significant_features = [
                        (feature, shap_value) for feature, shap_value in zip(feature_names, shap_values[0])
                        if abs(shap_value) > threshold
                    ]

                    # Identify the most influential feature from the filtered list
                    if significant_features:
                        max_influence_feature, max_shap_value = max(significant_features, key=lambda x: abs(x[1]))
                    else:
                        max_influence_feature, max_shap_value = None, None  # No significant features
                    ...
                    if max_influence_feature:
                        influence_direction = "positive" if max_shap_value > 0 else "negative"
                        print(f"Most influential feature ({influence_direction} impact): {max_influence_feature}, SHAP Value: {max_shap_value}")
                        print(f"Explanation: The agent took the action '{action_name}' primarily because the '{max_influence_feature}' feature had a {influence_direction} influence on the decision.\n")
                        if detailed_context:
                            print(f"Detailed Context: {detailed_context}\n")
                    else:
                        print("No significant SHAP values found for this action.\n")

                                        

                    print("Blue Last Action: " + str(cyborg.get_last_action('Blue')))
