        ...
        for red_agent in [B_lineAgent, RedMeanderAgent, SleepAgent]:
            ...
            for i in range(MAX_EPS):
                ...
                for j in range(num_steps):
                    ...
                    # Find the most influential feature and extract related context
                    for feature, shap_value in zip(feature_names, shap_values[0]):
                        if shap_value > 0 and (max_shap_value is None or shap_value > max_shap_value):
                            max_shap_value = shap_value
                            max_influence_feature = feature
                    ...
                    if max_influence_feature:
                        # Get the host related to the most influential feature (e.g., 'Enterprise2' from 'Enterprise2_Activity')
                        host = max_influence_feature.split('_')[0]
                        if host in anomalies:
                            # Get process info, session info, and file info from anomalies for that host
                            process_info = anomalies.get(host, {}).get('Processes', [])
                            session_info = anomalies.get(host, {}).get('Sessions', [])
                            file_info = anomalies.get(host, {}).get('Files', [])

                            # Build detailed context based on the type of anomaly detected
                            # Build detailed context based on the type of anomaly detected
                            if process_info:
                                detailed_context = f"Suspicious activity was detected on {host}, related to a process with PID {process_info[0].get('PID')}."
                            elif session_info:
                                detailed_context = f"Unusual access was detected on {host}, where user '{session_info[0].get('Username')}' may have unauthorized privileges."
                            elif file_info:
                                file_name = file_info[0].get('File Name')
                                file_density = file_info[0].get('Density')
                                detailed_context = f"Possible malware identified on {host}: '{file_name}', with a risk score of {file_density}."
...
# Ensure SHAP values match feature names when plotting
if len(shap_values_all[0]) == len(feature_names):
    shap.summary_plot(np.array(shap_values_all), feature_names=feature_names)
else:
    print(f"Error: SHAP values do not match the feature names length for plotting.")
