...
class PPOAgent(BaseAgent):
    def __init__(self, input_dims=52, action_space=[i for i in range(158)], lr=0.002, betas=[0.9, 0.990], gamma=0.99, K_epochs=4, eps_clip=0.2, restore=False, ckpt=None,
                 deterministic=True, training=True, start_actions=[]):
    ...
    def add_scan(self, observation):
        ...
        # Adjust to handle 2D observation arrays
        if observation.ndim == 2:
            observation = observation[0]  # Take the first row if it's a 2D array
        
        for id, index in enumerate(indices):
            # Perform element-wise comparison using np.any() or np.all()
    ...
    def set_initial_values(self, action_space, observation=None):
        ...
        if self.restore:
            pretained_model = torch.load(self.ckpt, map_location=lambda storage, loc: storage, weights_only=True)
